...
- **`ext/simdjson/simdjson_ruby.cpp`** — The entire C extension. Defines the `Simdjson` module with a `parse` method and `Simdjson::ParseError` exception. The `make_ruby_object()` function recursively converts simdjson DOM types (ARRAY, OBJECT, INT64, UINT64, DOUBLE, STRING, BOOL, NULL_VALUE) to Ruby equivalents. Also defines `Simdjson::Builder`, a thin wrapper over `simdjson::builder::string_builder` (a low-level, structure-agnostic JSON serializer) and its `Simdjson::BuilderError` exception. The builder is a `TypedData` object created in `#initialize`. `#append(obj)` serializes a whole value via `append_value()`, which dispatches Ruby values (nil/bool/Integer/Float/String/Symbol, and recursively Array→array and Hash→object) to the right `string_builder` call; integers outside int64 range go through their decimal string via `append_raw`, and `buffer` (aliased as `to_s`) copies the currently-buffered bytes into a UTF-8 Ruby string. For incremental output there is a stateful writer API — `push_object`/`push_array`/`push_value`/`push_key`/`pop`/`pop_all` — that inserts commas and colons automatically: a `std::vector<frame>` nesting stack on `builder_state` tracks, per open container, whether a child has been emitted (comma) and whether a key awaits its value (colon), so the caller never manages punctuation. Constructed as `Builder.new(io = nil, buffer_size: 4096)`; `buffer_size` is the internal buffer's size (initial allocation always, flush threshold when streaming). Given an `io` (any `#write`-responder), the builder becomes a streaming writer: `builder_maybe_flush()` writes the buffer to the io and resets it once it passes `buffer_size`, and `#flush` forces the remainder out — so a large document is delivered in bounded chunks rather than held whole. The byte buffer's `clear()` leaves the nesting stack intact, so flushing mid-document is safe. Builder + io + buffer_size + stack live on a `builder_state` wrapper whose io is GC-marked. **Important:** `simdjson.h` must be included before `ruby.h` — Ruby's `subst.h` redefines `snprintf` which breaks `std::snprintf` in simdjson.
